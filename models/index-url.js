...
const updateUrl = async (updateData, whereData = {}) => {

    await update(TABLE_NAME, updateData, whereData);
}


const removeAllUrl = async () => {
    await remove(TABLE_NAME);
}

const removeInactiveUrl = async () => {

    await remove(TABLE_NAME, {actual_flag: 0});
}
